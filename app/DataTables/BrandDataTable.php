...
class BrandDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                $deleteBtn = "<a href='" . route('admin.brand.destroy', $query->id) . "' class='btn btn-danger delete-item ml-2'><i class='far fa-trash-alt'></i></a>";

                return $editBtn . $deleteBtn;
            })
            ->addColumn('logo', function ($query) {
                return $img = "<img width='50px' src='" . asset($query->logo) . "'/>";
            })
            ...
            ->addColumn('is_featured', function ($query) {
                if ($query->is_featured == 1) {
                    $bagde = '<span class="badge badge-success">Yes</span>';
                } else {
                    $bagde = '<span class="badge badge-danger">No</span>';
                }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('logo'),
            Column::make('name'),
            Column::make('is_featured')->width(100)->title('Featured'),
